...
const deployFunction: unknown = async function (hre: HardhatRuntimeEnvironment) {
  ...
  dim("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
  dim("Synapse Network Contest - Deploy Script");
  dim("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
  ...
  const whitelistContract = await ethers.getContractAt("ContestSelector", contestDeployResult.address);
  ...
  if (linkBalance < linkData.fee) {
    ...
    const transaction = await linkContract.connect(signers[0]).transfer(contestDeployResult.address, linkData.fee, options);
