...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  dim("\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
  dim("           PublicKRSelector - Deploy Script");
  dim("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");

  dim(`network: ${chainName(chainId)} (${isTestEnvironment ? "local" : "remote"})`);
  dim(`deployer: ${deployer}`);

  cyan("\nDeploying PublicKRSelector...");

  const linkData = chainLinkData(chainId);

  const whitelistDeployResult = await deploy("PublicKRSelector", {
    from: deployer,
    args: [linkData.vrfCoordinator, linkData.linkAddress, linkData.keyHash, linkData.fee],
    skipIfAlreadyDeployed: true,
  });

  displayResult("PublicKRSelector", whitelistDeployResult);

  const whitelistContract = await ethers.getContractAt("PublicKRSelector", whitelistDeployResult.address, signer);
